...
for (iter in 1:100) {
      ...
      catch.eff.n1 <- runif(n1,0.5,2)
      catch.eff.n2 <- runif(n2,0.5,2)
      catch.eff.n3 <- runif(n3,0.5,2)
      ...
      set.seed(200)
      data1 <- sapply(1:n1, function(j) {
        phi1[idx1] <- 8 * rbinom(G1, size = 1, prob = 0.7)
        lambda_g1  <- phi1 * lambda0 * catch.eff.n1[j]
        (1 - rbinom(G, size = 1, prob = 0)) * rnbinom(G, size = 0.1, mu = lambda_g1)
      } )
      data2 <- sapply(1:n2, function(j) {
        phi2[idx2]  <- 8 * rbinom(G2, size = 1, prob = 0.7)
        lambda_g2 <- phi2 * lambda0 * catch.eff.n2[j]
        (1 - rbinom(G, size = 1, prob = 0)) * rnbinom(G, size = 0.1, mu = lambda_g2)
      } )
      data3 <- sapply(1:n3, function(j) {
        phi3[idx3]  <- 8 * rbinom(G3, size = 1, prob = 0.7)
        lambda_g3 <- phi3 * lambda0 * catch.eff.n3[j]
        (1 - rbinom(G, size = 1, prob = 0)) * rnbinom(G, size = 0.1, mu = lambda_g3)
      } )
